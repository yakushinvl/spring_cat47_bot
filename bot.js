...
bot.on('message_created', async (ctx) => {
    ...
    // --- ADMIN ACTIONS ---
        ...
        try { await bot.api.sendMessageToUser(app.initiator_id, ui.myApplications.statusNotification(appId, 'требует корректировки', text), { format: 'markdown' }); } catch (e) {}
...
// ACTIONS - Referral Link Creation
...
bot.action(/set_zone:(.+)/, async (ctx) => {
    ...
    if (state.data.ref_purpose) {
        ...
        // Явно передаем пустой массив в attachments, чтобы убрать кнопки выбора зоны
...
bot.action('my_applications', async (ctx) => {
    ...
    // Если много заявок, лучше не редактировать первую, а слать новые, 
    // но мы можем отредактировать "главное сообщение", если оно было.
    // Однако здесь цикл по приложениям, так что оставим ctx.reply для списка, 
    // но начальное сообщение (если мы из меню) можно было бы заменить.
    // Для простоты оставим как есть, но добавим кнопку назад в каждое.
...
bot.action(/cancel_app:(\d+)/, async (ctx) => { 
    ...
    // Требование 4: Отмена только до решения службы безопасности
...
bot.action(/execute_action:(\d+):(.+)/, async (ctx) => {
    ...
    const app = await db.getApplicationById(ctx.match[1]);
    try { await bot.api.sendMessageToUser(app.initiator_id, ui.myApplications.statusNotification(ctx.match[1], ctx.match[2]), { format: 'markdown' }); } catch (e) {}
});
